...
def run(data_size,batch_size,validation_status, n_clusters):
    try:
        ...
        labels = birch.fit_predict(data_pca)


        if validation_status == "1":
            silhouette = " | Silhouette Score: " + str(silhouette_score(data_pca, labels))
        else:
            silhouette = " "
...
#     size=int(size+100)
#     b_size=int(size/10)
#     with open('birch_results.txt', 'a') as file:
